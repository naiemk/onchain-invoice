...
- Trustless Commerce (shop invoices, merchant/customer UI, commerce sweeper product) lives in the sibling repo `trustless-commerce` (`/workspaces/trustless-commerce`). It uses the **Commerce** contracts (`contracts/commerce/`) and `CommerceInvoiceSdk` — not the classic Forwarder→Receiver path. Do not fold that product app into this library.
