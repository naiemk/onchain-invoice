...
import {ITronForwarder} from "./interfaces/ITronForwarder.sol";
import {TronForwarder} from "./TronForwarder.sol";
import {TronClones} from "./proxy/TronClones.sol";

contract TronInvoiceSweeper {
    struct SweepCall {
        bytes32 invoiceId;
        address token;
        bytes data;
    ...
    constructor(address receiver_) {
        ...
        receiver = receiver_;
        forwarderImplementation = address(new TronForwarder(receiver_));
    }

    function getInvoiceAddress(bytes32 invoiceId) public view returns (address) {
        return TronClones.predictDeterministicAddress(
    ...
    function createInvoice(bytes32 invoiceId) public returns (address forwarder) {
        ...
        forwarder = TronClones.cloneDeterministic(forwarderImplementation, _invoiceSalt(invoiceId));
        emit InvoiceCreated(invoiceId, forwarder);
    }

    function sweepTrx(bytes32 invoiceId, bytes calldata data) public returns (address forwarder, uint256 amount) {
        forwarder = createInvoice(invoiceId);
        amount = ITronForwarder(forwarder).sweepTrx(invoiceId, data);
    ...
    function sweepToken(
        ...
        forwarder = createInvoice(invoiceId);
        amount = ITronForwarder(forwarder).sweepToken(token, invoiceId, data);
    ...
    function bulkExecute(SweepCall[] calldata calls) external returns (uint256[] memory amounts) {
        ...
        for (uint256 i; i < length;) {
            ...
            (, amounts[i]) = call_.token == address(0)
                ? sweepTrx(call_.invoiceId, call_.data)
